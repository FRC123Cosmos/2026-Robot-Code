...
public class MAXSwereveModule {
    ...
    private final SparkFlex kTurningFlex;
    ...
    public MAXSwereveModule(int turningCANid, int drivingCANid, double angleOffset) {
        kTurningFlex = new SparkFlex(turningCANid, MotorType.kBrushless);
        kDrivingFlex = new SparkFlex(drivingCANid, MotorType.kBrushless);


        kDriveEncoder = kDrivingFlex.getEncoder();
        kTurningEncoder = kTurningFlex.getAbsoluteEncoder();
        
        kTurningClosedLoopController = kTurningFlex.getClosedLoopController();
        ...
        kTurningFlex.configure(Configs.MAXSwereveModule.turningConfig, 
        ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
